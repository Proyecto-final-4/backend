...
public class SavingsGoalController {
    ...
    @PutMapping("/{id}")
    public GoalResponse update(@PathVariable UUID id, @RequestBody GoalUpdateRequest request) {
        String email = SecurityContextHolder.getContext().getAuthentication().getName();
        return savingsGoalService.update(email, id, request);
    }
    ...
    public void delete(@PathVariable UUID id) {
        String email = SecurityContextHolder.getContext().getAuthentication().getName();
        savingsGoalService.delete(email, id);
    }
}
